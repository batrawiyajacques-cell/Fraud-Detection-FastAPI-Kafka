...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("🗑️ Gestion des Révocations")
    ...
    try:
        ...
        df_delete = pd.read_sql_query("SELECT id_client FROM UTILISATEUR WHERE id_client != 100 ORDER BY id_client ASC;", conn)
        ...
        with st.form("Formulaire Suppression", clear_on_submit=True):
            ...
            if bouton_supprimer:
                if id_cible in [1, 2]:
                    ...
                    conn = psycopg2.connect(DB_URI)
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM UTILISATEUR WHERE id_client = %s;", (id_cible,))
                    conn.commit()
                    cursor.close()
                    conn.close()
...
# ==========================================
tab1, tab2, tab3 = st.tabs(["📊 Tableaux de Bord", "👥 Gestion des Tenants (Clients)", "⚙️ Pipeline Kafka & Audit"])
...
with tab1:
    ...
    try:
        ...
        try:
            total_val = int(df_stats['total'].iloc[0]) if not df_stats.empty else 0
        except:
            total_val = 0
        c1.metric("Transactions Évaluées", total_val)
        
        try:
            fraudes_raw = df_stats['fraudes'].iloc[0]
        ...
        try:
            risque_raw = df_stats['risque_moyen'].iloc[0]
